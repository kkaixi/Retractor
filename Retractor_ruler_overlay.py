...
# get videos
videos = glob.glob(directory + '*\\Videos\\* 400mm Excursion 16mm.avi')
for v in videos:
    i = table.loc[v[42:45], 'Knee Excursion Time'] + table.loc[v[42:45], 't0']
    if np.isnan(i):
        continue
    else:
        i = int(i)
    ...
    writer = imageio.get_writer(directory + v[42:45] + ' Knee Excursion.jpg')
    writer.append_data(reader.get_data(i))
    writer.close()
    
    # read it in pillow and ovelay the ruler
    im = Image.open(directory + v[42:45] + ' Knee Excursion.jpg').convert(mode='RGBA')
